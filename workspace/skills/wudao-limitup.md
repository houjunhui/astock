...
1. **检查 Skill 是否有新版本**  
   - 执行：`curl -s "$LB_API_BASE/skills/wudao-limitup/version"`  
   ...
   - 将远程 `version` 与本文档 YAML 头部的 **`version:`** 比较（建议语义化版本）。若**远程更新**，则覆盖本地本文件：  
     `curl -s -o <本文件实际路径> "$LB_API_BASE/skills/wudao-limitup/skill.md"`  
...
```bash
curl -s "$LB_API_BASE/skills/wudao-limitup/version"
```

若返回的 `version` 比本文件 YAML 里的 `version` 更新，则：

```bash
curl -s -o THIS_FILE_PATH "$LB_API_BASE/skills/wudao-limitup/skill.md"
```
...
| `industry` | 所属行业 |
| `jiuyangongshe_category_name` | 韭研概念分类 |
| `jiuyangongshe_analysis` | 韭研解读 |
