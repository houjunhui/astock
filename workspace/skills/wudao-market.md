...
1. **检查 Skill 是否有新版本**  
   - 执行：`curl -s "$LB_API_BASE/skills/wudao-market/version"`  
   - 响应为 JSON，取其中的 **`version`** 字符串（与 `success` 同级）。勿使用 `/skills/version` 多包汇总接口与本文件对比。  
   - 将远程 `version` 与本文档 YAML 头部的 **`version:`** 比较（建议按语义化版本）。若**远程更新**，则覆盖本地本文件：  
     `curl -s -o <本文件实际路径> "$LB_API_BASE/skills/wudao-market/skill.md"`  
   - 下载后**以新文件中的 Endpoints 为准**再继续调用 API。  
2. **环境变量**：若缺少 `LB_API_KEY` 或 `LB_API_BASE`，先提示用户按下方 **Setup** 配置，不要猜测 Base URL。  
...
```bash
curl -s -o THIS_FILE_PATH "$LB_API_BASE/skills/wudao-market/skill.md"
```

将 `THIS_FILE_PATH` 换成本地实际路径（如 `.cursor/skills/wudao-market.md`）。
...
**注意**：盘中"今日"K线来自东方财富实时数据，可能缺少 `pre_close`/`change`/`pct_chg`。

### 3. 分时数据

分钟级分时数据，来自东方财富实时 API。
...
返回东方财富原始数据格式。核心数据在 `data.trends` 数组中，每条是**逗号分隔的字符串**（非 JSON 对象）：
