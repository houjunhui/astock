...
from astock.quicktiny import get_ladder, get_auction_for_codes, get_market_overview_fixed, get_kline_hist
...
def get_day_prices(code, date_str):
    """获取当日OHLC价格（用get_kline_hist，速度快）"""
    try:
        klines = get_kline_hist(code, days=2)
        if not klines:
...
def simulate_day(date_str, params=None, capital=CAPITAL, verbose=True):
    ...
    for code, ad in ads.items():
        ...
        if chg <= 0 or not price or price <= 0:
            continue
        # amount=None时（历史竞价数据无金额字段），跳过金额过滤
        if amount is not None and amount < params.get("auction_amount_min", 50_000_000):
            continue
    ...
    candidates = sorted(candidates, key=lambda x: -x["lb"])  # DEBUG
    print(f"[DEBUG] candidates after sort+limit: {len(candidates)}")
    ...
    # ── 6. 模拟买入 ──
    day_buys = []  # DEBUG
    print(f"[DEBUG] entering simulate_buy loop, candidates={len(candidates)}")
    for c in candidates:
        day_buys.append({
    ...
    print(f"[DEBUG] getting prices for {code}")
        ...
        print(f"[DEBUG] close_price is None for {buy["code"]}")
            continue
